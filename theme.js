...
document.addEventListener('DOMContentLoaded', function() {
 ...
 // FORZARE VIDEO SU MOBILE
document.addEventListener('DOMContentLoaded', () => {
    const video = document.querySelector('.hero-video');
    if (video) {
        // Proviamo a farlo partire forzatamente
        video.play().catch(error => {
            console.log("Autoplay bloccato dal browser o risparmio energetico attivo.");
        });
    }
});
...
document.getElementById('search-btn').onclick = async () => {
    ...
    try {
        ...
        // Creiamo lo stesso stile degli spot fissi
        resultDiv.innerHTML = `
            <div class="spot-item" style="border-left: 5px solid #ffcc00; background: rgba(255, 204, 0, 0.1);">
                <div class="spot-info">
                    <strong>${data.name} <span class="spot-temp" style="color:#ffcc00">${Math.round(data.main.temp)}°C</span></strong>
                    <small>Meteo attuale: ${data.weather[0].description}</small>
                </div>
                <span class="status-badge" style="background:#ffcc00; color:#000">LIVE</span>
            </div>`;
    } catch (err) {
        resultDiv.innerHTML = `<p style="color: #ef4444; font-size: 0.8rem;">Località non trovata. Riprova.</p>`;
    }
};